...
#ifndef CAutoDokingCamera_H
#define CAutoDokingCamera_H

#include <opencv2/opencv.hpp>
#include <opencv2/imgproc/imgproc.hpp>
#include <opencv2/imgproc/imgproc_c.h>

#include "CMotorsCom.h"
#include "CLogger.h"
...
namespace NAAS
{
	...
	/** Autodocking class: This class endows to the robot with the capability of to perform an
	  *		autonomous docking.
	  */

	class CAutoDocking : CLoggeable
	{
	public:
		...
		/** Launch the execution of the autodoking.
		  */
		void run();

		/** Change the exection motors mode, i.e, if the autodocking sends real
		  *	movements commands or not.
		  *	\param noMotors true if you want to activate the no motors mode, false to deactivate.
		  */
		void changeNoMotorsMode( const bool &noMotors );

		/** Get the current noMotorMode state.
		  * \return True if no motors mode is activated, false in otherwise.
		  */
		bool getNoMotorsMode(){ return m_no_motors_mode; }
	...
	private:
		...
		size_t	m_reactive_navigation_algorithm; //!< Used reactive navigation algorithm. Currently there is only one

		bool	m_no_motors_mode; //!< If we want to debug the application it could be interesting not perform movements (don't use the motors)!
		...
		/** Variables for HW interaction */

		CMotorsCom *m_motors;	//!< Reference to a motors communication object!

		/** State variables */

		bool	m_run, m_stop, m_started;	//!< Variables to enconde the module state
		...
		struct movement
		{
			...
			mrpt::utils::TTimeStamp	time;			//!< When was it ordered?
		}m_last_movement; //!< Information relative to the las movement performed
		
		CTimeLogger	m_timeLog;	//!< To take measures about execution time
		...
		{
			vector<vector_int>	m_hsv_thresholds;	//!< Thresholds for the HSV segmentation
		...
		struct turnCompleteTester 
		{
			...
			{ 
				float new_orientation = abs(RAD2DEG( orientation ));
		...
		static void dummyAutoDocking( CAutoDocking *obj );
		...
		/** A simple reactive navigation method. */
		size_t simpleReactiveNavigation();
