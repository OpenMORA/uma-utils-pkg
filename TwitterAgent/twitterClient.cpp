...
bool CTwitterClient::IsConnected()
{
	return connected;
}
bool CTwitterClient::Connect(std::string user,std::string pass)
{
	printf("\n[TwitterClient]:Connecting...%s,%s",user.c_str(),pass.c_str());
	bool res=true;
	std::string replyMsg;
    ...
    twitterObj.getOAuth().setConsumerKey( std::string( "0Q6D1kr8cEdaAUZhENxw" ) );
    twitterObj.getOAuth().setConsumerSecret( std::string( "51NqWqHxfKHX9BwBmVvdRRql56dNtXENXtGJklEeG8" ) );
	
    // Step 1: Check if we alredy have OAuth access token from a previous run  
    std::string myOAuthAccessTokenKey("986960598-5vzHJjalssC2tEx8QmPr4mpRc7g13RL8MDE5ETp6");
    std::string myOAuthAccessTokenSecret("amItCsdtcm7lqHju92pcoT4cocgqDcfz2B1XeYl8G8");
...
bool CTwitterClient::Tweet(std::string msg)
{
